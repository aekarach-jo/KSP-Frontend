...
const NavUserMenuDropdownToggle = React.memo(
  React.forwardRef(({ onClick, expanded = false, user = {} }, ref) => {
    ...
    return (
      ...
      >
        <img className="profile" alt={ResName?.avatar} src={`${userLogin?.avatar ? userLogin?.avatar : '/img/userEmpty/account.png'}`} />
        <div className="name">{`${ResName?.firstName || ''}${' '}${ResName?.lastName || ''}`}</div>
      </a>
    );
  })
);
