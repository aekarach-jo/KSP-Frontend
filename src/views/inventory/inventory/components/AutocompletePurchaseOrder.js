...
const AutocompletePurchaseOrder = ({ onSelect, inputProps }) => {
  ...
  const searchItem = async (e, keyword) => {
    ...
    const res = await request({ url: API.FIND_PURCHASE_ORDER, params: { limit: 10, no: keyword } });
    ...
    return (
      <Row className="g-0 position-relative">
        ...
        <Col className="d-flex align-items-center">
          <div className="stretched-link body-link">{no}</div>
        </Col>
      </Row>
    );
  };
...
export default AutocompletePurchaseOrder;
