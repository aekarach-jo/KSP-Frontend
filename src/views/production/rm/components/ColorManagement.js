...
import AddModal from './ColorManagementAdd';
...
const ColorManagement = ({ show, onHide }) => {
  ...
  const translate = useMemo(
    () => ({
      ...
      field: {
        code: f({ id: 'common.field.code' }),
        abbr: f({ id: 'rm.field.abbr' }),
        name: f({ id: 'rm.field.color' }),
  ...
  const { useColorList, useColorpSaveMutation } = useMaterialLovData();
  const { data, isLoading } = useColorList();

  const handleSaveSuccess = useCallback(() => {
    toast(translate.common.success, { className: 'success' });
  }, [translate.common.success]);

  const { mutateAsync: save, isLoading: isSaving } = useColorpSaveMutation({ onSuccess: handleSaveSuccess });
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: 'name',
        headerClassName: 'text-alternate text-uppercase col-5',
      },
      // { accessor: 'type', hide: true },
      {
        Header: '',
        id: 'action',
        ...
        <OverlayScrollbarsComponent options={{ overflowBehavior: { x: 'hidden', y: 'scroll' } }} className="scroll-track-visible">
          <Row className="mb-3">
            ...
            <Col md className="text-end mb-1">
              {/* <Button onClick={toggleAddModal}>{translate.common.add}</Button> */}
          ...
          <Row className="mb-3">
            <Col xs="12">
            <TableBoxed tableInstance={tableInstance} customStyle={customStyle} rowStyle={rowStyle} />
