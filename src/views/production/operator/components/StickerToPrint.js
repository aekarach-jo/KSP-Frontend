...
const StickerToPrint = React.forwardRef(({ tableInstance, stickerAmount, loading, componentRef, rowProps = () => ({}), isConfirmModal }, ref) => {
  ...
  useEffect(() => {
    if (tableInstance.productPiecePerPack !== undefined) setRowCount(15 * Math.floor(stickerAmount / tableInstance?.productPiecePerPack / 15));
  }, [stickerAmount]);
  return (
    <div ref={componentRef} style={{ width: '35cm', height: '21cm' }} className={isConfirmModal ? clx('react-table') : 'd-none'}>
      {/* <Table> */}
            ...
            return (
              ...
              >
                ...
                <tr style={{ height: '10mm', fontSize: '12px' }}>
                  ...
                  <td style={{ width: '2.1cm', border: '1px solid', textAlign: 'center' }}>{rowCount}</td>
                  <td style={{ width: '1.62cm', border: '1px solid', fontWeight: 'bold', paddingLeft: '4px' }}>Customer</td>
                  <td style={{ width: '6cm', border: '1px solid', fontSize: '12px', textAlign: 'center' }}>{tableInstance?.customer?.[0]?.customerName}</td>
                </tr>
                <tr style={{ height: '10mm', fontSize: '12px' }}>
                  ...
                  <td style={{ width: '6cm', border: '1px solid', paddingBottom: '0mm', fontSize: '12px', paddingLeft: '4px' }}>
                    {tableInstance?.productionOrderNo?.value}
                  </td>
                </tr>
              </Col>
            );
