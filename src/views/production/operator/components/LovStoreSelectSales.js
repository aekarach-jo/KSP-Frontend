...
function mapOptions(lovList, lovLabel, lovValue, isCode, value, POType) {
  if (isCode) {
    lovLabel = 'code';
    lovList = lovList?.filter((data) => data.id !== value);
  }
  const filterSale = lovList.filter((data) => data.isSale);
  return (filterSale || []).map((lov) => ({
    label: lov[lovLabel],
    value: lov[lovValue],
    detail: lov,
  }));
...
export default LovStoreSelectSales;
