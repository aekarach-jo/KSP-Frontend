...
import ButtonsCheckAll from 'components/table/components/ButtonsCheckAll';

const Title = ({ title, description,setShowModal, setProductId, isLoading, addButton, buttons = {}, tableInstance = {}, showCheckAll }) => {
      ...
      <Row>
        ...
        <Col xs="12" sm="12" md="auto" className="d-md-block">
          ...
          {buttons?.submit && !buttons?.submit?.isHide && (
            <Button variant="primary" className="btn-icon btn-icon-start w-100 w-md-auto mb-1" disabled={isLoading} onClick={() => buttons?.submit?.onSubmit()}>
              <CsLineIcons icon="send" /> <span>{buttons?.submit?.label || 'Submit'}</span>
            </Button>
          )}
          {buttons?.print && !buttons?.print?.isHide && (
            <Button variant="primary" className="btn-icon btn-icon-start w-100 w-md-auto mb-1" disabled={isLoading} onClick={() => buttons?.print?.onPrint()}>
              <CsLineIcons icon="print" /> <span>{buttons?.print?.label || 'Submit'}</span>
            </Button>
          )}
          {addButton && (
