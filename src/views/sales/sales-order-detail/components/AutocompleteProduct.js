...
const AutocompleteProduct = ({ onSelect, inputProps, editData, customerId, reference }) => {
  const [valueState, setValueState] = useState('');
  const [suggestions, setSuggestions] = useState([]);
  const [data, setData] = useState([]);

  useEffect(() => {
    request({ url: `${SERVICE_URL}/saleOrderExp/findCO`, params: { customerId, reference } }).then((res) => {
      // setData(res?.data?.data);
      const test = res.data.data;
      console.log(
        test
          .map((item) => item.detail.map(({ id, product, amount, price }) => ({ id, amount, price, no: item.no, code: product?.code, name: product?.name })))
          .flat()
      );
