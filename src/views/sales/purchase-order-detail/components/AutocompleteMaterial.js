...
const AutocompleteMaterial = ({ onSelect, inputProps, editData }) => {
  ...
  // }, []);
  useEffect(() => {
    if (editData?.material?.id) {
      setValueState(editData?.material?.code);
  ...
  const searchPurchaseItem = async (e, keyword) => {
    ...
    e.target.blur();
    const res = await request({ url: `${SERVICE_URL}/masterData/material/find`, params: { limit: 10, code: keyword } });
    setData(res?.data?.data);
    e.target.focus();
  };
