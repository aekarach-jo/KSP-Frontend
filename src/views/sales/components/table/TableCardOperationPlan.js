...
const TableCard = ({
  ...
  const renderHeader = ({ tableInstance: tableInstanceHeader, filter: filterCpn }) => {
    const { gotoPage, setFilter, filter } = tableInstanceHeader;

    const onSetFilter = (dataResult) => {
      const result = dataResult.map((item) => item.value).join(',');
      setFilter({ ...filter, sortMultiCondition: result, page: 0 });
      gotoPage(0);
    };
        ...
        <Col sm="12" md="7" lg="8" xxl="9" className="text-end">
          ...
          <div className="d-flex justify-content-end align-items-end gap-1 text-start">
            {filterCpn && (
              <>
                <Select
                  className="multiFilter w-100 w-sm-50"
