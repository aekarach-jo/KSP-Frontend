...
import CardContainer from './components/Table';
import TablePagination from './components/TablePagination';
import ResponsiveNav from './components/ResponsiveNav';
import TableDataNotFound from './components/TableDataNotFound';
import TableDataNotFoundInventory from './components/TableDataNotFoundInventory';
...
const TableMultiPurchase = ({ setShowModal, setProductId, tabs = [], onTabChange, activeKey, ...tableConfig }) => {
  ...
  const renderContainerProduct = ({ isLoading, tableInstance: tableInstanceContainer, tableInstanceProduct }) => {
    return (
      <div
      ...
      >
        <Row>
          <Col xs="12">
            ...
            <CardContainer setShowModal={setShowModal} setProductId={setProductId} className="react-table rows" tableInstance={tableInstanceProduct} />
            <TableDataNotFoundInventory tableInstance={tableInstanceContainer} tableInstanceProduct={tableInstanceProduct} />
          </Col>
          <Col xs="12">
  ...
  const renderContainer = ({ isLoading, tableInstance: tableInstanceContainer, tableInstanceProduct }) => {
    return (
      <div
        className={classNames({
          'overlay-spinner': isLoading,
        ...
        <Col>
          {tabs.length > 0 ? (
            <Tab.Container defaultActiveKey="first">
              ...
              <Nav variant="tabs" className="nav-tabs-title nav-tabs-line-title pt-5 mx-3" activeKey={activeKey} as={ResponsiveNav}>
                {tabs.map(({ eventKey, label }) => (
                  <Nav.Item key={eventKey}>
                    <Nav.Link eventKey={eventKey} onClick={() => onTabChange && onTabChange(eventKey)}>
                      {label}
                    </Nav.Link>
                  </Nav.Item>
                ))}
              ...
              <Tab.Content>
                {tabs.map(({ eventKey, isLoading, tableInstance: tableInstanceTab, tableInstanceProduct }) => (
                  <Tab.Pane key={eventKey} eventKey={eventKey}>
                    {renderContainer({ tableInstance: tableInstanceTab, isLoading, tableInstanceProduct })}
                  </Tab.Pane>
                ))}
              </Tab.Content>
            </Tab.Container>
...
export default TableMultiPurchase;
